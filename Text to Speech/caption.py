...
class Caption():
    ...
    def __init__(self, context, cc_type, name, text, start_time, end_time, accent, channel, pitch, reconstruct=False):
        ...
        if reconstruct: # reconstructing on fileload
            ...
            if self.frame_start != -1:
                self.sound_strip, self.filename = tts.sound_strip_from_text(context, text, pitch, self.frame_start, accent, channel)
            else:
                self.sound_strip, self.filename = tts.sound_strip_from_text(context, text, pitch, 0, accent, channel)
